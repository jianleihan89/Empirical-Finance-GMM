...
simulation=1;                                ###1: do simulation; 0 otherwise
NREP=1000;                                      ##number of repetitions for Monte-Carlo simulation
simalpha=0;                              ##Set up a alpha for power calculation
set.seed(4325);
...
###substract alpha
alpha=c(0.00634495645157456,0.00266034665008419,0.00105234672277137,0.000851763333120069,0.000770881305111647,0.000849070785098423,0.000693446063836431,0.0000661058176683947,0.000600917573799473,-0.0000732867365830011)

for (i in 1:10){
  data1[,i+3]=data1[,i+3]-alpha[i]+simalpha;
}
originaldata=data1;
########End of User Defined Settings###########################
###objective function2: restricted model with NR parameters
FUN2=function(PR,data1){                                  ###PR:Parameters for restricted model, for example, 10 betas
...
OBJ2=function(PR,data1){
  return(FUN2(PR,data1)$obj)
}
###function1: unrestricted model with N parameters
FUN1=function(PU,data1){                                  ###PU: Parameters for unrestricted model, for example, 10 alphas and 10 betas
  Z=matrix(0,nrow(data1),M);
...
OBJ1=function(PU,data1){
  return(FUN1(PU,data1)$obj)
}
...
Dmatrix=function(P,restrict,data1){
  
  if (!restrict){
    D=matrix(0,M,N);
    for (i in 1:M){
        dfun=function(pu){
          return(FUN1(pu,data1)$moments[i])
        }
        D[i,]=grad(dfun,P)
    }
  }
    ...
    for (i in 1:M){
       dfun=function(pu){
        return(FUN2(pu,data1)$moments[i])
      }
      D[i,]=grad(dfun,P)
    }
  }
...
optpu=optimx(iniPU,OBJ1,method="nlminb",data1=data1); ##optimizer "nlminb
PU1=as.matrix(optpu[1:N]);
G1=FUN1(PU1,data1)$residual;                            ##store all residuals
...
WEGR=WEG;
optpu2=optimx(iniPU,OBJ1,method="nlminb",data1=data1); ## second step optimize for unrestricted model

optpr2=optimx(iniPR,OBJ2,method="nlminb",data1=data1); ## second step optimize for restricted model
## store estimated coefficients and residuals
Coef1=as.matrix(optpu2[1:N]);
Z1=FUN1(Coef1,data1)$residual;
Coef2=as.matrix(optpr2[1:NR]);
Z2=FUN2(Coef2,data1)$residual;
## calculate new weight matrix
WE1=WEM(Z1);
WE2=WEM(Z2);
##Call Dmatrix to calculate D
DU=Dmatrix(Coef1,restrict=FALSE,data1);
DR=Dmatrix(Coef2,restrict=TRUE,data1);
...
return(list("Ju"=Ju,"Jr"=Jr,"tu"=tu,"tr"=tr,"Coef1"=Coef1,"Coef2"=Coef2))}
}
###writeoutput
Jstore=rep(0,NREP)
for (k in 1:NREP){
  data1=originaldata[sample(nrow(originaldata),replace=TRUE),]
  Jstore[k]=gmm(data1,onlyJ=TRUE)
  print(k)
}
if (simulation==1){ 
  write.csv(Jstore,"Jstats.csv",row.names=FALSE);
}
if (simulation==0){
if (DIFSTAT==1){
  tp=1-pt(results$tu,NOBS-M);
  outdata=cbind(t(results$Coef1),t(results$tu),t(tp));
  J=matrix("",nrow(outdata),1);
  J[1,1]=results$Ju;
  outdata=cbind(outdata,J);
  outdata=data.frame(outdata);
  names(outdata)[3:4]=c("P-value","J-stat");
  tp=1-pt(results$tr,NOBS-NR);
  tr2=c(results$tr,rep("",N-NR));
  tp=c(tp,rep("",N-NR));
  co2=c(results$Coef2,rep("",N-NR));
  J[1,1]=Jr;
  outdata=cbind(outdata,co2,tr2,tp,J);
  J[1,1]=1-pchisq(results$Jr,M-NR);
  outdata=cbind(outdata,J);
  ...
  {
  tp=1-pt(results$tr,NOBS-NR);
  outdata=cbind(t(results$Coef2),t(results$tr),t(tp));
  J=matrix("",nrow(outdata),1);
  J[1,1]=results$Jr;
  outdata=cbind(outdata,J);
  J[1,1]=1-pchisq(results$Jr,M-NR);
