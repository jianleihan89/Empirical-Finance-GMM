setwd("C:/Users/Administrator/Dropbox/GMM R code") ##set work directory
...
N=11;                                         ###Number of parameters to be estimated (unrestricted), if there's no unrestricted model, define it same to restrited model
NR=11;                                        ###Number of parameters to be estimated (restricted)
...
if (DIFSTAT==0) N=NR;  
iniPU=c(rep(1,N-1),0);      ##set up initial parameter guess for unrestricted model. If no unrestricted model, same to restricted
iniPR=c(rep(1,NR-1),0);                       ##set up initial parameter guess for restricted model
#read input
...
FUN2=function(PR){                                  ###PR:Parameters for restricted model, for example, 10 betas
  ...
  for ( i in 1:10){                                 ### moment conditions for restricted model
    Z[,i]=as.matrix(data1[,3+i])-(1-PR[i])*PR[11]-PR[i]*data1$vwretd;}
  for ( i in 11:20){
...
if (DIFSTAT==0) {FUN1=FUN2; OBJ1=OBJ2;N=NR;}     ### if there is no unrestricted model to be estimate, set unrestricted model same to restricted model
