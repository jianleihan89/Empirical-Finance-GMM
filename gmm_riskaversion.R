...
N=1;                                         ###Number of parameters to be estimated (unrestricted), if there's no unrestricted model, define it same to restrited model
NR=1;                                        ###Number of parameters to be estimated (restricted)
M=10;                                         ###Number of Moment conditions 
...
ratiom<-(1+data1$vwretd)/(1+data2$bid/1200);
ratio1<-(1+data1$d1)/(1+data2$bid/1200);
ratio2<-(1+data1$d2)/(1+data2$bid/1200);
ratio3<-(1+data1$d3)/(1+data2$bid/1200);
ratio4<-(1+data1$d4)/(1+data2$bid/1200);
ratio5<-(1+data1$d5)/(1+data2$bid/1200);
ratio6<-(1+data1$d6)/(1+data2$bid/1200);
ratio7<-(1+data1$d7)/(1+data2$bid/1200);
ratio8<-(1+data1$d8)/(1+data2$bid/1200);
ratio9<-(1+data1$d9)/(1+data2$bid/1200);
ratio10<-(1+data1$d10)/(1+data2$bid/1200);

data1<-cbind(data1,ratiom,data1,ratio1,ratio2,ratio3,ratio4,ratio5,ratio6,ratio7,ratio8,ratio9,ratio10);
...
FUN2=function(PR){                                  ###PR:Parameters for restricted model, for example, 10 betas
  Z=matrix(0,nrow(data1),M);                               ### moment conditions for restricted model
  Z[,1]<-(data1$ratio1-1)*(data1$ratiom^PR)
  Z[,2]<-(data1$ratio2-1)*(data1$ratiom^PR)
  Z[,3]<-(data1$ratio3-1)*(data1$ratiom^PR)
  Z[,4]<-(data1$ratio4-1)*(data1$ratiom^PR)
  Z[,5]<-(data1$ratio5-1)*(data1$ratiom^PR)
  Z[,6]<-(data1$ratio6-1)*(data1$ratiom^PR)
  Z[,7]<-(data1$ratio7-1)*(data1$ratiom^PR)
  Z[,8]<-(data1$ratio8-1)*(data1$ratiom^PR)
  Z[,9]<-(data1$ratio9-1)*(data1$ratiom^PR)
  Z[,10]<-(data1$ratio10-1)*(data1$ratiom^PR)
...
FUN1=function(PU){                                  ###PU: Parameters for unrestricted model, for example, 10 alphas and 10 betas
  ...
  Z[,i]=as.matrix(data1[,3+i])-PU[i]-PU[i+10]*data1$vwretd;} 
  for ( i in 11:20){
  Z[,i]=Z[,i-10]*data1$vwretd;}
  g=colMeans(Z);
  Q=0;
  for (i in 1:M){
