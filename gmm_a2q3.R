...
M=21;                                         ###Number of Moment conditions 
...
FUN2=function(PR){                                  ###PR:Parameters for restricted model, for example, 10 betas
  ...
  for ( i in 1:10){                                 
    Z[,i+10]=as.matrix(PR[i]*((data1$vwretd-data1$rf)^2-PR[11]^2)-(data1$vwretd-data1$rf-PR[11])*(data1[,3+i]-data1$rf));}
  Z[,21]=as.matrix(data1$vwretd-data1$rf-PR[11]);
  g=colMeans(Z);
  Q=0;
  for (i in 1:M){
