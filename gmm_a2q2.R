...
N=2;                                         ###Number of parameters to be estimated (unrestricted), if there's no unrestricted model, define it same to restrited model
NR=2;                                        ###Number of parameters to be estimated (restricted)
...
iniPR=c(1,10);                       ##set up initial parameter guess for restricted model
#read input
...
data3=read_csv("a2q1.csv");
data3=data3 %>% mutate(deltac=c/lag(c));  ##consumption
data1=subset(data1,Date>19590100);
data3=subset(data3,month<2003);
data1$deltac=data3$deltac;
data1=data1 %>% mutate(lagdeltac=lag(deltac));
data1=data1 %>% mutate(lagrm=lag(vwretd)-lag(rf));
data1=na.omit(data1);
NOBS=nrow(data1);
...
FUN2=function(PR){                                  ###PR:Parameters for restricted model, for example, 10 betas
  ...
  for ( i in 1:5){                                 ### moment conditions for restricted model
    Z[,i]=as.matrix((PR[1]*(data1[,2+2*i]-data1$rf+1)*data1$deltac^(-PR[2])-1)*data1$lagdeltac);}
  for ( i in 1:5){                                 
    Z[,i+5]=as.matrix((PR[1]*(data1[,2+2*i]-data1$rf+1)*data1$deltac^(-PR[2])-1)*data1$lagrm);}
  g=colMeans(Z);
  Q=0;
  for (i in 1:M){
